...
- **pi-ai (`@earendil-works/pi-ai`) is the chat/agent backbone.** Both providers (Gemini and Codex) go through pi-ai's compatibility `getModel` + `complete` API. No more hand-rolled SSE clients, no more provider branching inside `geminiService.ts` / `agentService.ts` -- pi-ai owns transport, streaming, tool-call formatting, and (for Codex) the internal `chatgpt.com/backend-api/codex/responses` endpoint. Tool schemas use TypeBox (`Type.Object({...})`); pi-ai validates them at the provider boundary.
- **OAuth is also pi-ai's job.** `src/codexOAuth.ts` registers `openaiCodexProvider()` with `createModels()` and uses the provider auth contract for login, refresh, and bearer-token derivation. PKCE, loopback bind, state verification, and refresh-token rotation all live upstream. Do NOT reimplement these primitives in-house.
- **The wrappers bridge ESM-only dependencies into our CJS build.** `src/piAiClient.ts`, `src/codexOAuth.ts`, `src/geminiService.ts`, and `src/liveSttProvider.ts` use `importEsm()` so tsc cannot rewrite dynamic imports to `require()`. Don't replace these runtime imports with static imports; that crashes with ERR_REQUIRE_ESM.
...
- **Cancelling an in-flight Codex sign-in.** `models.login()` accepts the caller AbortSignal, while a manual-code prompt also carries its own signal that is aborted when the loopback callback wins. `src/codexOAuth.ts` combines both signals and leaves loopback cleanup to the provider. The loopback is bound to fixed port 1455 with no fallback. `src/main.ts` keeps the pending-login slot occupied for an extra ~250ms cushion (`PORT_RELEASE_CUSHION_MS`) so a re-bind from the next attempt doesn't race the kernel. Treat the cushion as heuristic, not a guarantee.
...
- **Preload bridge is auto-injected.** The default `preloadInjectionIntegration` ships a sandbox-safe preload via `registerPreloadScript`. Do **NOT** add `import '@sentry/electron/preload'` to `src/preload.ts` — the app runs `sandbox: true` (Electron 39 default) where arbitrary `require()` in a preload fails at runtime.
