...
- **Final-stage quality pass (the main investment — live is a light safety net only).** After the batch transcript is assembled, `transcribeWithTwoSteps` (a) runs `analyzeAssembledTranscript` over the joined text with segment headers/`---` separators stripped — this is what catches cross-segment loops (H3) that per-segment gating can't see, and stripping the scaffolding matters because adjacent headers from silent segments would false-flag as near-duplicates; (b) piggybacks a suspected-artifact review onto the EXISTING summary call (`TRANSCRIPT_QUALITY_PROMPT_BLOCK` + `transcriptQualityNotes` JSON key) — zero extra API calls, notes-only. Verdicts persist as `customFields.transcriptQuality = { analyzer?, modelNotes? }` in meta.json, only when something was found. **An LLM text-cleanup stage was tried and REMOVED by user decision (2026-07-22)**: asked to rewrite a transcript with artifacts deleted, the cleanup model itself entered a repetition loop (20,349-char input → 61,318-char single-line output). Audio-level retries + marking are the retained strategy — do not reintroduce text-level rewriting. Full pipeline ground truth: `docs/transcription-pipeline.md`.
