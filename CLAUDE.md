...
## Codex Provider Stack

- **OAuth flow is delegated to `@earendil-works/pi-ai/oauth`.** PKCE, loopback bind, state verification, refresh-token rotation -- all of it lives upstream. Do NOT reimplement these primitives in-house. The wrapper at `src/codexOAuth.ts` stays thin: lazy ESM load (`new Function('return import(...)')` because the package is ESM-only), typed facade (`resolveCodexAccessToken`, `requireCodexAccessToken`, `loginCodexOAuth`), and env-fallback credentials. New OAuth features land in the wrapper, never by patching pi-ai locally.
- **Why this direction.** A hand-rolled OAuth flow against an undocumented ChatGPT endpoint is a moving target -- the auth contract, loopback discovery, and refresh semantics change without notice. Delegating concentrates that upstream-change risk into one pinnable dep we can audit/upgrade as a unit.
- **Upgrade checklist for `@earendil-works/pi-ai`.** Before bumping the version: (1) loopback bind stays on 127.0.0.1 (not 0.0.0.0), (2) PKCE + state are enforced on the callback, (3) the API surface (`loginOpenAICodex`, `getOAuthApiKey`) is unchanged or the wrapper is updated to match, (4) refresh-token rotation behavior is documented in the changelog (we cache the rotated creds via `onCredentialsChanged`).
- **Codex Responses endpoint is our risk to own.** `https://chatgpt.com/backend-api/codex/responses` is OpenAI's internal endpoint and is NOT covered by pi-ai. We hit it directly from `src/openaiCodexClient.ts`. Accept that it can break without notice; own the fallback story (graceful error surface in the renderer + a suggestion to switch to the Gemini provider). When that happens, file against ChatGPT/OpenAI -- not pi-ai.
