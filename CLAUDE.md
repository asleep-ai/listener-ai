...
- **Quality gate wiring.** Batch paths (`transcribeSingleSegment`, `getShortAudioTranscript`) strip the `[NO_SPEECH]` sentinel, then run `applyTranscriptQualityGate`: flagged output triggers exactly ONE retry with `QUALITY_RETRY_TRANSCRIPT_PROMPT` (context-cleared: no glossary, no positional prefix, no examples) at `QUALITY_RETRY_TEMPERATURE` 0.7 — Whisper's fallback prescription: low temperature is what locks a decoder into self-reinforcing loops, so the evidence-triggered retry samples its way out while first attempts stay at 0.2 (codex diarize has no temperature knob and keeps provider defaults). A clean retry — including empty, which is evidence of silence — replaces the first result; a still-flagged retry keeps the FIRST result and logs it as uncertain. Detection alone never deletes text. `TranscriptionOptions.qualityRetry: false` (used by `transcribeLiveSnippet`) disables the retry: re-sending the same low-signal live blob buys no new evidence. `DEFAULT_TRANSCRIPT_PROMPT` intentionally has NO example dialogue (transcript-like sample text is a hallucination seed on silent audio) and defines the `[NO_SPEECH]` sentinel — don't re-add examples.
