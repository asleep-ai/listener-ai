...
- **pi-ai (`@earendil-works/pi-ai`) is the chat/agent backbone.** Both providers (Gemini and Codex) go through pi-ai's compatibility `getModel` + `complete` API. No more hand-rolled SSE clients, no more provider branching inside `geminiService.ts` / `agentService.ts` -- pi-ai owns transport, streaming, tool-call formatting, and (for Codex) the internal `chatgpt.com/backend-api/codex/responses` endpoint. Tool schemas use TypeBox (`Type.Object({...})`); pi-ai validates them at the provider boundary. **Known debt:** upstream marks the `/compat` entrypoint as temporary (slated for deletion with its ModelManager migration); `codexOAuth.ts` already uses the real `createModels()` API, so plan the same move for `piAiClient.ts` before the pi-ai bump that drops `/compat`. `getModel` falls back to the provider default (with a console warning) when a user-configured model id is missing from the bundled registry -- pi-ai bumps retire catalog ids, and a stale config must not brick every summary/agent call.
...
- **Upgrade checklist for `@earendil-works/pi-ai`.** Before bumping the version: (1) OAuth loopback bind stays on 127.0.0.1, (2) PKCE + state are enforced on the callback, (3) the compatibility API and provider-auth surface used by `piAiClient.ts` / `codexOAuth.ts` still exist, (4) refresh-token rotation behavior is documented in the changelog, and (5) the bundled registry still contains Listener's configured model ids.
