...
- **Repetition/hallucination guard (issue #182).** `src/transcriptQuality.ts` is the shared detector for ASR loop output on silence/noise/music: consecutive near-identical lines (char-bigram Dice ≥ 0.9 on whitespace-stripped text, so Korean spacing variants compare equal), 1-4-word block loops, space-less character-period loops (KMP), and a local deflate compression metric (deliberately NOT labeled a provider compression_ratio). Thresholds are conservative so legitimate repetition (`네, 네`, stutters, emphasis, short chants) never flags; calibration criteria live in the issue. Logs carry metrics/reasons only — never transcript text.
- **Quality gate wiring.** Batch paths (`transcribeSingleSegment`, `getShortAudioTranscript`) strip the `[NO_SPEECH]` sentinel, then run `applyTranscriptQualityGate`: flagged output triggers a bounded retry LADDER with `QUALITY_RETRY_TRANSCRIPT_PROMPT` (context-cleared: no glossary, no positional prefix, no examples) at `QUALITY_RETRY_TEMPERATURES` [0.4, 0.8] — Whisper's escalating-fallback prescription compressed to two rungs: low temperature is what locks a decoder into self-reinforcing loops, the low rung keeps recovered speech accurate when it suffices, the high rung maximizes escape. Rungs run in order, first analyzer-clean result wins, throwing rungs are skipped; first attempts stay at 0.2 (codex diarize has no temperature knob and keeps provider defaults). Production calibration note (2026-07-21, real 1h meeting): 0.2 retries failed to recover looped segments, 0.7 recovered 3/4. A clean retry — including empty, which is evidence of silence — replaces the first result; a still-flagged retry keeps the FIRST result and logs it as uncertain. Detection alone never deletes text. `TranscriptionOptions.qualityRetry: false` (used by `transcribeLiveSnippet`) disables the retry: re-sending the same low-signal live blob buys no new evidence. `DEFAULT_TRANSCRIPT_PROMPT` intentionally has NO example dialogue (transcript-like sample text is a hallucination seed on silent audio) and defines the `[NO_SPEECH]` sentinel — don't re-add examples.
...
- **Batch energy gate (audio-evidence silence skip).** Before any provider call, `measureMaxVolumeDb` (ffmpeg `volumedetect`) checks the segment/file peak; below −50 dBFS (`SILENT_SEGMENT_MAX_VOLUME_DB`) the audio is provably silent and is never uploaded — a silent segment keeps its header with an empty body, a silent whole file surfaces the friendly no-speech error, a silent live snippet resolves to `''`, all at zero API cost. Measurement failure (missing ffmpeg, unparseable output) FAILS OPEN to normal transcription, so the gate can only ever skip provable silence, never quiet speech.
...
- **Final-stage quality pass (the main investment — live is a light safety net only).** After the batch transcript is assembled, `transcribeWithTwoSteps` (a) runs `analyzeAssembledTranscript` over the joined text with segment headers/`---` separators stripped — this is what catches cross-segment loops (H3) that per-segment gating can't see, and stripping the scaffolding matters because adjacent headers from silent segments would false-flag as near-duplicates; (b) runs `cleanTranscriptWithModel` — an LLM cleanup pass on the app's small/cheap tier (gemini → `geminiFlashModel`, codex → `codexModel`; temperature 0, reasoning low) that removes STT artifacts from the assembled transcript. **The model is the final judge by explicit user decision (2026-07)** — do not replace this with more heuristics. Fail-open on any error (original kept), empty responses rejected, `transcriptOnly`/CLI transcript output stays raw, one extra small-model call per full transcription. **The cleaner's own output is gated too**: cleanup is deletion-only by contract, so output longer than input is rejected, and the analyzer vetoes output flagged worse than what it was asked to clean — the cleanup model can catch the very loop disease it treats (observed in production 2026-07-22: 20,349-char input returned as a 61,318-char single-line loop at temperature 0); (c) piggybacks a suspected-artifact review onto the EXISTING summary call (`TRANSCRIPT_QUALITY_PROMPT_BLOCK` + `transcriptQualityNotes` JSON key) — zero extra API calls, notes-only. Verdicts persist as `customFields.transcriptQuality = { analyzer?, modelNotes?, modelCleanup? }` in meta.json, only when something was found. Known trade-off, accepted: text-only judgment can't verify audio grounding, which is why the audio-evidence layers (energy gate, bounded audio retry) stay in front of it.
- **Live duplicate suppression (safety net).** `LiveSessionService` drops a final whose normalized text exactly matches the previous final within 30s (min 6 normalized chars) — the shape a provider stuck on silence emits every commit/window. Fuzzy/overlap dedupe is intentionally absent: without timestamp or audio evidence it risks deleting real speech. Flagged-but-kept finals carry `segment.quality = { flagged, reasons }` for consumers. Still open in #182: renderer-side VAD/energy gating for live chunk uploads (the batch-side energy gate covers live snippets only after upload), and opt-in quality-triggered provider fallback (needs both providers configured + a config surface — keep it a separate change).
