...
- **Final-stage quality pass (the main investment — live is a light safety net only).** After the batch transcript is assembled, `transcribeWithTwoSteps` (a) runs `analyzeAssembledTranscript` over the joined text with segment headers/`---` separators stripped — this is what catches cross-segment loops (H3) that per-segment gating can't see, and stripping the scaffolding matters because adjacent headers from silent segments would false-flag as near-duplicates; (b) piggybacks a suspected-artifact review onto the EXISTING summary call (`TRANSCRIPT_QUALITY_PROMPT_BLOCK` + `transcriptQualityNotes` JSON key) — zero extra API calls, notes-only. Verdicts persist as `customFields.transcriptQuality = { cleaned?, uncertainSegments?, analyzer?, modelNotes? }` in meta.json when cleanup was accepted, uncertain text was kept, or another quality finding exists. **Decision history.** A broad LLM cleanup pass was removed by user decision on 2026-07-22 after it expanded a 20,349-character transcript into a 61,318-character repetition loop. On 2026-07-23, the user reintroduced only the ladder-exhausted per-segment cleanup described above, guarded by never-grow and same-verdict-clean acceptance. The assembled-transcript analyzer and summary piggyback remain notes-only and never rewrite transcript text. Full pipeline ground truth: `docs/transcription-pipeline.md`.
- `transcriptQuality` is internal diagnostic metadata: retain it in `meta.json` and raw JSON output, but exclude it from app tabs/copy output, rendered Markdown/CLI output, and Notion pages. Other custom fields remain user-visible.
