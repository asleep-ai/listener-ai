...
- **Final-stage quality pass (the main investment — live is a light safety net only).** After the batch transcript is assembled, `transcribeWithTwoSteps` (a) runs `analyzeAssembledTranscript` over the joined text with segment headers/`---` separators stripped — this is what catches cross-segment loops (H3) that per-segment gating can't see, and stripping the scaffolding matters because adjacent headers from silent segments would false-flag as near-duplicates; (b) piggybacks a suspected-artifact review onto the EXISTING summary call (`TRANSCRIPT_QUALITY_PROMPT_BLOCK` + `transcriptQualityNotes` JSON key) — zero extra API calls, and the model is told to exclude suspected artifacts from the summary but never rewrite the transcript. Both verdicts persist as `customFields.transcriptQuality = { analyzer?, modelNotes? }` in meta.json, only when something was found. A text-only model judge was deliberately NOT added as a separate call: it cannot verify audio grounding (only the bounded audio retry can), and a dedicated small-model judge adds cost/latency for weaker evidence.
