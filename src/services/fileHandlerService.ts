...
export class FileHandlerService {
  ...
  registerHandlers(): void {
    ...
    ipcMain.handle(
      ...
      async (_, fileData: { name: string; dataBase64: string }): Promise<FileResult> => {
        try {
          this.ensureDirectoryExists();
    ...
    ipcMain.handle(
      'copy-audio-file',
      async (_, fileData: { sourcePath: string; name: string }): Promise<FileResult> => {
        try {
          this.ensureDirectoryExists();
          ...
          if (stats.size > this.maxFileSize) {
            throw new Error(
              `File too large: ${(stats.size / 1024 / 1024).toFixed(2)}MB (max: 500MB)`,
            );
          }
