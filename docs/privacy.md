...
- **OpenAI (Codex / Whisper)** — transcription, summarization. When you configure Listener.AI to use Codex, audio and prompts are sent to OpenAI using your own OAuth credentials. Your use is governed by OpenAI's privacy policy.
- **Google Drive sync (optional)** — when you enable Drive sync, Listener.AI signs you in to Google using the **`https://www.googleapis.com/auth/drive.file`** scope. This scope restricts the app to files it creates in your Drive; Listener.AI cannot see your other files, folders, or content. Synced transcripts (and, if you opt in, audio recordings) are stored in a "Listener.AI" folder in your own Google Drive. Deletions you perform in the app are propagated to Drive's trash for the standard recovery window.
